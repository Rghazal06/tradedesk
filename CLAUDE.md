...
- `/quotes` — List + create quotes. HST auto-calc. Optional deposit amount. Edit + delete. Portal link generation.
...
- `/jobs` — Jobs list. Photo gallery + lightbox. Voice notes. AI report. Checklist (customizable checkbox items per job, shown on the AI report and public share page too). Time tracking (clock in/out per job, optional crew attribution). Job costing panel — despite earlier docs calling this a "profit bar," it's actually 4 stat cards (Material Spend from linked receipts / Labor Cost from tracked time × hourly-rate crew / Quoted subtotal from the most recent matching quote / Over-Under difference, materials only) plus a receipts table — there is no visual progress bar.
