...
- `/portal?token=` — **Single-quote view only** (`app/portal/page.tsx`), token-based via `quotes.portal_token`. Shows one quote's line items + approve button, and now a deposit-payment step if `deposit_amount` is set. Still on Tailwind classes (predates the inline-styles-only rule) — a known inconsistency, not yet cleaned up. There is no unified multi-tab hub with Overview/Quotes/Invoices/Jobs and no `/api/portal/data` route — an earlier version of this doc described that and it never existed in the codebase.
