...
export default function NewQuotePage() {
  ...
  const handleSaveQuote = async () => {
    ...
    const { error } = await supabase.from("quotes").insert({ user_id: user.id, customer_name: customerName, customer_email: customerEmail, customer_phone: customerPhone, job_description: jobDescription, line_items: normalizedLineItems, subtotal, hst, total, notes: quoteNotes, status: "Draft", deposit_amount: depositValue });
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f5f5f4', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <div className="qn-body" style={{ padding: '32px', overflowY: 'auto', flex: 1 }}>
          ...
          {aiGenerated && (
            <div style={{ background: '#faf5ff', border: '1px solid #e9d5ff', borderRadius: '10px', padding: '14px 18px', marginBottom: '20px', display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
              <svg width="18" height="18" viewBox="0 0 15 15" fill="none" style={{ color: '#7c3aed', flexShrink: 0, marginTop: '1px' }}><path d="M7.5 1l1.2 5.3L14 7.5l-5.3 1.2L7.5 14 6.3 8.7 1 7.5l5.3-1.2z" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round"/></svg>
              <div>
                <p style={{ fontSize: '13px', fontWeight: '700', color: '#5b21b6', margin: '0 0 2px' }}>Generated by AI — review before sending</p>
                <p style={{ fontSize: '12px', color: '#7c3aed', margin: 0 }}>Check all line items, quantities, and prices. You are responsible for the final quote your customer receives.</p>
              </div>
            </div>
          )}
