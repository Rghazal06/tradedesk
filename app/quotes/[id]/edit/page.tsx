...
export default function EditQuotePage(): React.JSX.Element {
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      ...
      let data: any = {};
      try { data = await res.json(); } catch { setErrorMessage(`Server error (${res.status}).`); return; }
      ...
      setSuccessMessage('AI regenerated your line items. Review and adjust before saving.');
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f5f5f4', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      <style>{`
        ...
        @media (max-width: 768px) {
          .qe-topbar { padding: 14px 16px !important; flex-wrap: wrap !important; gap: 10px !important; }
          .qe-topbar a { font-size: 12px !important; }
          .qe-body { padding: 16px !important; }
          .qe-grid-2 { grid-template-columns: 1fr !important; }
          .qe-grid-2 > [style*="span 2"] { grid-column: span 1 !important; }
          .qe-ai-row { grid-template-columns: 1fr !important; }
          .qe-ai-row button { width: 100% !important; justify-content: center !important; }
          .qe-totals { max-width: 100% !important; margin-left: 0 !important; }
          .qe-actions { flex-wrap: wrap !important; }
          .qe-actions button, .qe-actions a { flex: 1 !important; text-align: center !important; justify-content: center !important; }
        }
      `}</style>
      <Sidebar activePath="/quotes" />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        {/* Top bar */}
        <div className="qe-topbar" style={{ background: 'white', borderBottom: '1px solid #e5e7eb', padding: '16px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <h1 style={{ fontSize: '20px', fontWeight: '700', color: '#111', margin: 0 }}>Edit Quote</h1>
            <p style={{ color: '#6b7280', fontSize: '13px', margin: '2px 0 0' }}>
              {customerName ? `for ${customerName}` : 'Update quote details below'}
            </p>
          </div>
        ...
        <div className="qe-body" style={{ padding: '32px', overflowY: 'auto', flex: 1 }}>
          ...
          <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '24px', marginBottom: '20px' }}>
            ...
            <div className="qe-grid-2" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }}>
              <div>
          ...
          {/* Status */}
          <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '24px', marginBottom: '20px' }}>
            <h2 style={{ fontSize: '15px', fontWeight: '700', color: '#111', margin: '0 0 16px' }}>Quote Status</h2>
              ...
              {['Draft', 'Sent', 'Approved', 'Declined'].map(s => (
                <button
                  ...
                  style={{
                    ...
                    border: quoteStatus === s ? '2px solid #16a34a' : '1px solid #e5e7eb',
                    background: quoteStatus === s ? '#f0fdf4' : '#f9fafb',
                    color: quoteStatus === s ? '#16a34a' : '#6b7280',
                  }}
                >
                  {s}
                </button>
          ...
          <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '24px', marginBottom: '20px' }}>
            ...
            <div className="qe-ai-row" style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '16px', alignItems: 'end', marginBottom: '16px' }}>
              <div>
          ...
          <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '24px', marginBottom: '20px' }}>
            ...
            {/* Totals */}
            <div className="qe-totals" style={{ marginTop: '16px', marginLeft: 'auto', maxWidth: '300px', background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '10px', padding: '16px' }}>
              {[
          ...
          {/* Actions */}
          <div className="qe-actions" style={{ display: 'flex', gap: '12px' }}>
            <button onClick={() => void handleSaveQuote()} disabled={isSaving} style={{
