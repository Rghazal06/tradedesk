                  ...
                  {quotes.map((quote, i) => (
                    <div key={quote.id} style={{ padding: '16px 20px', borderBottom: i < quotes.length - 1 ? '1px solid #f3f4f6' : 'none', background: selectedIds.has(quote.id) ? '#f0fdf4' : 'transparent' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                        ...
                        <button onClick={async () => { const anyQuote = quote as any; let token = anyQuote.portal_token; if (!token) { token = crypto.randomUUID(); await supabase.from('quotes').update({ portal_token: token }).eq('id', quote.id); } const link = `${window.location.origin}/portal?token=${token}`; await navigator.clipboard.writeText(link); setToast('Portal link copied — send it to your customer.'); setTimeout(() => setToast(''), 3000); }} style={{ padding: '12px', background: '#faf5ff', color: '#7c3aed', border: '1px solid #ddd6fe', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', minHeight: '48px' }}>Portal Link</button>
                        <button disabled={deletingId === quote.id} onClick={() => void handleDeleteQuote(quote.id)} style={{ gridColumn: 'span 2', padding: '12px', background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', minHeight: '48px', opacity: deletingId === quote.id ? 0.5 : 1 }}>Delete</button>
                      </div>
                    </div>
                  ))}
                </div>
