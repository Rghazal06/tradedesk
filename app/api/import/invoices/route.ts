...
export async function POST(req: NextRequest) {
  ...
  for (const row of rows) {
    ...
    const subtotalRaw = row.subtotal ? parseFloat(row.subtotal.replace(/[^0-9.]/g, '')) : null;
    const hstRaw = row.hst ? parseFloat(row.hst.replace(/[^0-9.]/g, '')) : null;
    const subtotal = subtotalRaw != null && !isNaN(subtotalRaw) ? subtotalRaw : Math.round((total / 1.13) * 100) / 100;
    const hst = hstRaw != null && !isNaN(hstRaw) ? hstRaw : Math.round((total - subtotal) * 100) / 100;

    const rawStatus = row.status?.toLowerCase().trim() || 'unpaid';
    const status = rawStatus.includes('paid') && !rawStatus.includes('unpaid') ? 'paid' : 'unpaid';
